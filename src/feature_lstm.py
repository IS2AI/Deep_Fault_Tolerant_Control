...
from models import RNN, FNN, SeRNN_FW, SeRNN_FWXX
...
def simulate(xs0, sys, model, win_len, f1, f2, f3, index, sim_normalize, sim_fault):
    ...
    for ind in range(sim_len):
        ...
        y, lstm_output = model(x_faulty_seq)

        lstm_feat = lstm_output[:, -1, :]
        lstm_feat = lstm_feat.cpu().detach().numpy()
        #print('--->>> ',  lstm_feat.shape)
        lstm_list.append(lstm_feat)
    ...
    return state_to_nn, inputs_nn, states_true, lstm_list


def plot_states(states_true, state_to_nn, states_ocp, inputs_nn, control_ocp, win_len, is_plot, x0, y0, lstm_feat):

    file_st = '../checkpoints/' + '_rnn_states_ALL_fault_yes_wheel10' +'.txt'
    ...
    lstm_arr = np.concatenate( lstm_feat, axis=0)
    print('--- ', lstm_arr.shape)
    ...
    file_st = '../checkpoints/' + '_rnn_feat_ALL_fault_yes_wheel10' +'.txt'

    with open(file_st, "w") as myCsv:
         csvWriter = csv.writer(myCsv, delimiter=' ')
         print(lstm_arr.shape)
         csvWriter.writerows(lstm_arr)
    ...
    if is_plot == True:
        ...
        lstm_arr = np.rot90(lstm_arr)
        im = plt.imshow(lstm_arr)
...
if __name__ == '__main__':
    ...
    load_model_name = 'Model_WW4XX_test2_ep200' #'Model_WW4XX_test_ep200'
    model = SeRNN_FWXX(1, device)
    model.to(device)
    model.eval()
    model.load_state_dict(torch.load('../checkpoints/' + load_model_name + '.pth'))
    ...
    np.random.seed(1)
    rand_fault_time = np.random.randint(70, size=sim_num) + 30 # 30 - 200
    ...
    file_cost = '../checkpoints/' + sim_model_name + '_JJ_cost_' + str(version) +'.txt'
    ...
    for index in range(sim_num):
        ...
        state_to_nn_list,inputs_nn_list,states_true_list, lstm_list_a = simulate(xs, sys, model, win_len, fault_time, fault_states, fault_val, index, sim_normalize, sim_fault)
        ...
        if cost_ocp>0:
            ...
            if is_plot == True:
                ...
                plot_states(states_true, state_to_nn, states_ocp, inputs_nn, control_ocp, win_len, is_plot, xs, ys, lstm_list_a)
